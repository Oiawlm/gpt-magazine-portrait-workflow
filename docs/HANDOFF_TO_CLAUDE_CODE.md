...
最新执行口径：当前稳定 MVP 使用 `assets/inbox/` 作为默认图片入口。普通用户把人物多角度照片放入 `assets/inbox/` 并触发工作流后，仓库和 skill 应使用默认目录、默认命名和第一轮默认 4 张的 MVP 策略；不要要求用户手动输入人物名、路径、张数或“是否开始”。纯拖图即跑仍是最终目标，只有当前 Codex 环境能把拖入图片附件暴露为本地路径时才走这条路线。触发语和放入 `assets/inbox/` 的照片本身视为执行授权，只有覆盖旧文件、缺前置能力、路径不存在或队列无法修复时才暂停。
