...
- 不使用 DeepSeek V4 Pro；不要把它写入任务队列、文本整理、fallback 或可选步骤。
- 不允许任何模型替代 Doubao-Seed-2.0-Pro 的图片理解职责，除非用户明确更换标准路线。
...
11. 第一轮默认生成 4 张；后续版本再支持用户自定义张数。
12. 更新人物 Markdown、任务状态和运行记录。

触发语和拖入照片本身视为执行授权；公开 MVP 不再二次询问人物名、路径、张数或是否开始。只有缺少 Codex 生图能力、Codex 生图工具服务器错误/超时/不可用、缺少 Doubao 接入、队列校验无法修复、输入路径不存在或输出会覆盖旧文件时，才暂停并说明原因。
