...
这个测试用于确认仓库下载后能正常执行 MVP 的基础脚本和队列校验。它不会调用 GPT Image，也不会生成图片。

## 前提

在仓库根目录执行以下命令。Windows PowerShell 可直接使用。仓库可以通过 `git clone` 获取，也可以从 GitHub 页面点击 **Code** -> **Download ZIP** 下载并解压。
...
当前稳定 MVP 使用 `assets/inbox/` 作为默认图片入口。需要验证脚本默认扫描能力时，可以临时把测试图片放到：

```text
assets/inbox/
```

脚本在没有 `-SourceImagePath` 参数时会自动扫描该目录。拖图即跑仍是最终目标；只有当前 Codex 环境能暴露拖图本地路径时才走 `-SourceImagePath` 路线。
