...
可选。未传入 SourceImagePath 时，默认扫描 assets/inbox/ 下的图片。
...
if (-not $SourceImagePath -or $SourceImagePath.Count -eq 0) {
    throw "未找到人物原始图片。Codex 如果能拿到拖入图片的本地路径，应通过 -SourceImagePath 传入；如果当前 Codex 环境不暴露拖图路径，请把 3-5 张图片放入 assets/inbox/ 后重试。"
}
