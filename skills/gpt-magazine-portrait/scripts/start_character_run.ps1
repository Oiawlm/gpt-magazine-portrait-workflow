...
可选。开发者自测或环境兜底时使用；未传入 SourceImagePath 时默认扫描 assets/inbox/。
...
if (-not $SourceImagePath -or $SourceImagePath.Count -eq 0) {
    throw "未找到人物原始图片。公开 MVP 的普通用户主流程是直接拖图给 Codex；Codex 应读取拖入图片的本地路径并通过 -SourceImagePath 传入。当前环境未提供可用图片路径，无法自动启动人物运行。assets/inbox/ 仅用于开发者自测或环境兜底，不应作为普通用户必做步骤。"
}
