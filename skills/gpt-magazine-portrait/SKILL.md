...
2. 调用 Doubao-Seed-2.0-Pro 读取人物图和风格图，生成提示词队列，任务队列文件统一输出到 `assets/characters/<人物名>/tasks/` 目录
3. 运行 `powershell -ExecutionPolicy Bypass -File .\skills\gpt-magazine-portrait\scripts\validate_queue.ps1 -QueuePath "任务队列路径"` 验证 JSON 格式
### 第三步：审核与确认
