...
10. 不要把 `assets/inbox/` 写成普通用户主流程；用户默认只需要把图片拖给 Codex。`assets/inbox/` 只允许作为开发者自测或环境兜底。
11. 不要假设 Codex Desktop 一定能把对话中的图片附件保存为本地文件。只有当前工具或上下文明确提供图片路径时，才能调用 `start_character_run.ps1 -SourceImagePath ...`。
...
4. 当前 Codex 环境能把用户拖入的图片附件暴露给 agent，至少能提供可传给脚本的本地文件路径。
5. 用户提供同一人物的多角度照片，至少包含正脸、45 度侧脸和正侧脸。
...
2. 从本轮用户拖入的图片附件、文件列表或 Codex 暴露的本地路径中提取 3-5 张人物照片路径。只有实际拿到文件系统路径后，才能进入下一步。
3. 拿到图片路径后运行：
...
4. 不要要求用户手动输入路径、复制文件或把图片放入 `assets/inbox/`。
5. 如果当前 Codex 环境完全不能把拖入图片暴露为本地路径，必须暂停并说明：“当前 Codex 环境无法读取拖入图片的本地路径，无法自动启动人物运行。”不要声称可以把对话图片自动保存到临时目录，也不要把这改写成用户需要额外操作。
6. 仅在开发者自测、无生图 quickstart 或用户明确要求兜底时，才可以使用 `assets/inbox/`：

```powershell
powershell -ExecutionPolicy Bypass -File .\skills\gpt-magazine-portrait\scripts\start_character_run.ps1
```

7. 如果用户没有提供人物名，使用脚本自动生成的 `character-YYYYMMDD-HHMMSS`。
8. 原始图固定保存到 `reference/originals/`；不要让用户手动选择目录。
9. 人物资料 Markdown 先由 Codex 根据可观察特征自动草拟；不要要求用户手动填写。
