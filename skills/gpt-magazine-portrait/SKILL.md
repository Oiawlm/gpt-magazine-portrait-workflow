...
description: 基于 GPT Image 的人物杂志写真自动化生成工作流，配合 gpt-magazine-portrait-workflow 资产仓库使用。用于用户要求按 gpt-magazine-portrait 工作流生成杂志写真、上传多角度人物图跑一轮写真、用仓库风格资产生成提示词队列、审核人物写真任务队列并准备生图时。
---
## 触发场景
...
### 第五步：执行生图
1. 按照用户确认的任务列表，逐条使用 GPT Image / ChatGPT Plus 生成图片
2. 每次最多提交3个任务，避免触发频率限制
3. 每生成一张图片，立即保存到任务指定的输出路径
4. 验证图片文件存在、大小正常、可以正常打开
5. 实时更新任务队列中的状态（pending → processing → completed/failed）

### 第六步：记录与迭代
1. 自动生成生成结果记录，使用 `templates/generation_result.template.md` 模板
2. 引导用户提供每张图片的反馈（相似度、风格匹配度、优缺点、评分）
3. 更新人物 Markdown 文档，添加生成的图片和评价信息
4. 批量更新任务队列的最终状态
5. 将用户反馈和有效规则沉淀到 `docs/PROMPT_RULES.md` 和人物资料中
6. 生成下一轮优化建议，供用户选择是否继续
## 工具依赖
