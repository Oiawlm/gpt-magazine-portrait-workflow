...
```powershell
powershell -ExecutionPolicy Bypass -File .\skills\gpt-magazine-portrait\scripts\start_character_run.ps1 -SourceImagePath "用户拖入图片1","用户拖入图片2","用户拖入图片3"
```

3. 如果用户没有提供人物名，使用脚本自动生成的 `character-YYYYMMDD-HHMMSS`。
4. 原始图固定保存到 `reference/originals/`；不要让用户手动选择目录。
5. 人物资料 Markdown 先由 Codex 根据可观察特征自动草拟；不要要求用户手动填写。
