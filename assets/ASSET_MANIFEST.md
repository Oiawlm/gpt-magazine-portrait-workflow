...
当前稳定 MVP 使用此目录作为默认图片入口。用户把同一人物的 3-5 张多角度照片放入此目录后，`start_character_run.ps1` 会在没有 `-SourceImagePath` 参数时自动扫描并启动人物运行。图片文件名可以任意，脚本按常见图片扩展名识别并复制重命名。拖图即跑仍是最终目标；只有当前 Codex 环境能暴露拖图本地路径时才走纯拖图路线。
